  ...
  <basic-container>
    ...
    >
      <template #leaderUserId="{row}">
        <div class="div">{{ row.leaderName }}</div>
      </template>
      <template #leaderUserIdForm="{row,disabled}">
        <el-select
          v-model="row.leaderUserId"
          filterable
          remote
          :disabled="disabled"
          placeholder="请选择 机构领导"
        ...
        >
          <el-option
            ...
            :label="item.realName"
            :value="item.id"
            @click.native="handlelLeaderSelect(row,item)"
          >
...
export default {
      ...
      option: {
        ...
        column: [
          ...
          },
          {
            label: "机构类型",
            type: "select",
            dicUrl: "/api/rabbit-system/dict/dictionary?code=org_category",
            props: {
              label: "dictValue",
              value: "dictKey",
            },
  ...
  methods: {
    ...
    handlelLeaderSelect(row, item) {
      row.leaderName = item.realName
    },
    handleAdd(row) {
    ...
    beforeOpen(done, type) {
      ...
      if (["add", "edit"].includes(type)) {
        ...
        this.form.oldParent = this.form.parentId;
        this.initData();
      }
      //没必要,已返回全部数据
