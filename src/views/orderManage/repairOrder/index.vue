  ...
  <div class="backstage-index">
    <el-card class="box-card">
      <div class="order-state">
        ...
        <div class="state">
          ...
          <span>已接收：</span>
          <span>{{ stateInfo.order_state_receive }}条</span>
        </div>
      </div>
      ...
      <div style="position: relative;">
        ...
        >
          ...
          <el-form-item label="工单编号：" v-show="show">
            <el-input
              style="width: 217px"
          ...
          <el-form-item label="总部协助：" v-show="show">
            <el-select
              style="width: 217px"
              v-model="form.hqHelp"
              @visible-change="visibleChange"
          ...
          <el-form-item label="工单发起时间：" v-show="show">
            <el-date-picker
          ...
          <el-form-item label="消缺负责人：" v-show="show">
            <my-select
              style="width: 217px"
              :options="eliminationManagerList"
              ref="selectRef2"
              v-model="form.eliminationManagers"
              @visible-change="visibleChange"
              @input="handleEliminationManagerInput"
              :optionProp="{ value: 'userId', label: 'userName' }"
            />
          </el-form-item>
          <el-form-item label="告警时间：" v-show="show">
            <el-date-picker
          ...
          <el-form-item label="设备名称：" v-show="show">
            <el-input
              style="width: 217px"
          ...
          <el-form-item label="设备编码：" v-show="show">
            <el-input
              style="width: 217px"
          ...
          <el-form-item label="告警级别：" v-show="show">
            <el-select
              style="width: 217px"
          ...
          <el-form-item label="有无电量损失：" v-show="show">
            <el-select
              style="width: 217px"
          ...
          <el-form-item label="故障类型：" v-show="show">
            <el-select
              style="width: 217px"
              v-model="form.malfunctionTypes"
              multiple
              collapse-tags
              @visible-change="visibleChange"
              placeholder="请选择"
          ...
          <el-form-item label="是否完成：" v-show="show">
            <el-select
              style="width: 217px"
              v-model="form.isAccomplish"
              @visible-change="visibleChange"
              placeholder="请选择"
            >
              <el-option :key="1" label="是" :value="1" />
          ...
          <el-form-item label="数据来源：" v-show="show">
            <el-select
              style="width: 217px"
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <i
                :class="
                  scope.row.isAccomplish == 1
                    ? 'el-icon-success'
                    : 'el-icon-error'
                "
                :style="{
                  color: scope.row.isAccomplish == 1 ? '#67C23A' : '#F56C6C'
                }"
              ></i>
            </template>
          </el-table-column>
    ...
    >
      <Position
        ref="positionRef"
        :Single="false"
        :mainStationIdList="form.stationIds"
        :subStationIdList="form.sonStationIds"
        :positionIdList="form.positionIds"
      />
...
import {
  queryMaintainList,
  querySumByOrderStatus,
  delMaintainByID,
  exportMaintainList
} from "@/api/order/repairOrder.js";
import { mapGetters } from "vuex";
export default {
  name: "Grouping",
  components: { mySelect, Position },
  data() {
    return {
      tableHeight: null,
      nowTime: "",
      ...
      form: {
        ...
        startTime: "", //工单开始时间
        endTime: "", //工单结束时间
        eliminationManagers: [], //消缺负责人
        initiators: [], //发起人
        hqHelp: "", //总部协助    是：1/否：0（默认“否”，可修改）
        positionIds: [], //设备ids
        equipmentName: "", //设备名称
        equipmentCode: "", //设备编码
        equipmentModel: "", //设备型号
        highAlarmLevels: [], //告警级别
        isPowerLoss: "", //有无电量损失
        malfunctionTypes: [], //故障类型
        isAccomplish: "", //是否完成
        dataSources: "" //据来源○系统    ○手动
      },
      initiatorList: [], // 发起人下拉
  ...
  watch: {
    "form.stationIds"(val) {
      this.getQuerySubStationDropDownList();
      this.form.sonStationIds = [];
    }
  },
  methods: {
    setUI() {
    ...
    getQuerySumByOrderStatus(data) {
      querySumByOrderStatus(data).then(res => {
    ...
    getQueryList(data) {
      queryMaintainList(data).then(res => {
    ...
    // 消缺负责人下拉
    getEliminationManagerList(data) {
      userList(data).then(res => {
        this.eliminationManagerList = res.data.data ? res.data.data : [];
      });
    },
    // 消缺负责人 - input
    handleEliminationManagerInput(val) {
      if (val) {
        this.getEliminationManagerList({
          userName: val,
          mainStationIdList: this.form.stationIds,
          inMyMainStation: 0
        });
    ...
    // 设备名称弹窗 - 添加
    equipmentAdd() {
      let oldIds = this.form.positionIds;
      this.equipmentVisible = false;
      let data = this.$refs["positionRef"].choosePosition.concat(
        this.$refs["positionRef"].showData
      );
      console.log(data);
      // this.form.positionIds = this.$refs["positionRef"].choosePosition.map(
      //   v => v.id
      // );
      // this.form.equipmentName = this.$refs["positionRef"].choosePosition
      //   .map(v => v.positionName)
      //   .join(",");
      this.form.positionIds = data.map(v => v.id);
      this.form.equipmentName = data.map(v => v.positionName).join(",");
      console.log(this.form.positionIds);
      if (this.form.positionIds !== oldIds) {
        this.searchData();
      }
    },
    // 设备名称弹窗 - 取消
    ...
    reset() {
      ...
      this.handleInput();
      this.$refs.selectRef2.value = [];
      this.$refs.selectRef2.searchText = "";
      this.handleEliminationManagerInput();
      this.searchData();
    },
    // 删除
    deletes() {
    ...
    // 新增按钮
    handleAdd() {
      this.$router.push({ path: "./newRepairOrder" });
    },
    // 导出
    handleExport() {
      let param = {
    ...
    alarmLevelFormat(row) {
      let result = row.highAlarmLevel;
      this.warningLevelOptions.forEach(ele => {
        if (ele.dictKey === row.highAlarmLevel) {
    ...
    sort(num) {
      if (num == 0) {
        ...
        this.sort1 == "no"
          ...
          : (this.sort1 = "no");
        this.orderColumn = "equipmentCode";
