    ...
    >
      <basic-container>
        ...
        >
          ...
          <!--权限名称-->
          <template #scopeNameForm="{ form }">
            <el-input
              clearable
              @input="handleScopeNameInput"
              @keyup.native="isEditScopeName = true"
              placeholder="请输入 权限名称"
              v-model="form.scopeName"
            />
          </template>
          <!--权限编号-->
          <template #resourceCodeForm="{ form }">
            <el-input
              clearable
              @input="handleSourceCodeInput"
              @keyup.native="isEditResourceCode = true"
              placeholder="请输入 权限编号"
              v-model="form.resourceCode"
            />
          </template>
          <!-- 权限字段-->
          <template #scopeColumnForm="{ form }">
            <el-input
              clearable
              @input="handleScopeColumnInput"
              @keyup.native="isEditScopeColumn = true"
              placeholder="请输入 权限字段"
  ...
  data() {
    const validateSymbol = (rule, value, callback) => {
      ...
      if (!regExp.test(value)) {
        callback(new Error("值应只包含英文字母、数字、. 、 - 和 *"));
        return
      }
      callback()
    }
    ...
    return {
      ...
      isEditResourceCode: false,
      isEditScopeColumn: false,
      isEditScopeName: false,
      menuClass: '',
      drawerVisible: false,
      direction: 'rtl',
      scopeMenuId: 0,
      ...
      option: {
        ...
        column: [
          ...
          },
          {
            ...
            prop: "parentId",
            type: "tree",
            dicUrl: "/api/rabbit-system/menu/tree",
            hide: true,
            props: {
              label: "title"
            },
      ...
      optionScope: {
        ...
        column: [
          ...
          },
          {
            label: "规则类型",
            type: "select",
            dicUrl: "/api/rabbit-system/dict/dictionary?code=data_scope_type",
            props: {
              label: "dictValue",
              value: "dictKey"
            },
  ...
  methods: {
    initScope() {
      ...
      this.$refs.crudScope.option.column.filter(item => {
        if (watchMode) {
          if (item.prop === "scopeName" && !this.isEditScopeName || this.formScope.scopeName === "") {
            this.formScope.scopeName = `${this.scopeMenuName} [${name}]`;
          } else if (item.prop === "resourceCode" && !this.isEditResourceCode || this.formScope.resourceCode === "") {
            this.formScope.resourceCode = this.scopeMenuCode;
          } else if (item.prop === "scopeColumn" &&
            scopeType !== 5 ||
            this.formScope.scopeColumn === "") {
            this.formScope.scopeColumn = column;
          }
        }
        if (item.prop == 'scopeClass') {
          item.disabled = !!this.menuClass
        } else if (item.prop === "scopeValue") {
          item.display = scopeType === 5;
        }
      });
    },
    handleScopeNameInput(value) {
      if (value === "") {
        this.isEditScopeName = false;
      }
    },
    handleScopeColumnInput(value) {
      if (value === "") {
        this.isEditScopeColumn = false;
      }
    },
    handleSourceCodeInput(value) {
      if (value === "") {
        this.isEditResourceCode = false;
      }
    },
    // 菜单管理模块
    ...
    onLoad(page, params = {}) {
      this.loading = true;
      getLazyMenuList(this.parentId, Object.assign(params, this.query))
        .then(res => {
    ...
    handleDataScope(row) {
      ...
      this.menuClass = row.category == 3 && row.path ? (row.path.split("/")[2]) : ''
    ...
    beforeOpenScope(done, type) {
      if (["add"].includes(type)) {
        if (this.menuClass) {
          this.$set(this.formScope, 'scopeClass', this.menuClass)
        }
        this.watchMode = true;
        this.initScope();
      }
