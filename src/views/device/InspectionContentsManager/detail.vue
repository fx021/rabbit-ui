              ...
              >
                <template scope="scope">
                  <el-input
                    ...
                    v-model="scope.row.checkDatagName"
                    style="width: 300px"
                    placeholder="请输入"
                  >
                  </el-input>
...
export default {
  ...
  mounted() {
    ...
    editEquipmentDetail({ id: this.$route.query.id }).then((res) => {
      this.form = res.data.data;
      this.getmainStationId = this.form.mainStationId;
      this.getsubid =
        this.form.equipmentCheckSubStationResponseList[0].subStationId;
      console.log(this.getsubid, "sub");
      this.subStationOptions.forEach((v) => {
        if (
          this.form.equipmentCheckSubStationResponseList[0].subStationId ==
          v.subStationId
        ) {
          this.form.subStationName = v.subStationName; //展示所属组织
        }
      });
      this.changeRediod = this.form;
      this.changeRediods = this.form;
      this.form.isEffective = this.form.isEffective == 1 ? "是" : "否";
      this.tableDatat = res.data.data.equipmentCheckItemResponseList;
      let arr = [];
      this.dialogformx.equipmentTypeList = this.form.equipmentType;
      this.getsubStationIdx = this.getsubid;

      console.log(this.form, "xxxx");
      res.data.data.equipmentCheckPositionResponseList.forEach((v) => {
        arr.push(v.positionName);
      });
      this.positionNames = arr;
      this.form.changeRediodx =
        res.data.data.equipmentCheckPositionResponseList;
    });
  },
  filters: {
  ...
  methods: {
    ...
    deletes() {
      this.$confirm("操作无法撤回，确定要删除吗?").then(() => {
        removeEquipmentTemplate({
          equipmentCheckIdList: [this.$route.query.id],
        }).then((res) => {
    ...
    sure() {
      ...
      this.tableDatat[indexs].checkDatagId = arr2.toString();
      this.tableDatat[indexs].checkDatagName = arr.toString();
      ...
      this.$refs.form.validate((v) => {
        if (v) {
          editEquipmentTemplate({
            ...
            editEquipmentCheckSubStationRequestList:
              this.form.equipmentCheckSubStationResponseList,
            checkName: this.form.checkName,
            equipmentType: this.dialogformx.equipmentType,
            isEffective: this.form.isEffective == "是" ? 1 : 0,
            editEquipmentCheckItemRequestList: this.tableDatat,
            editEquipmentCheckPositionRequestList: this.form.changeRediodx,
          }).then((res) => {
            console.log(res);
            if (res.data.code == "200") {
              this.$message({
    ...
    principalAdd() {
      ...
      if (this.changeRediod.hasOwnProperty("mainStationId")) {
        ...
        this.form.equipmentCheckSubStationResponseList = [
          {
            mainStationId: this.changeRediod.mainStationId,
            subStationId: this.changeRediod.id,
          },
        ];
