<template>
  <div class="operation-staging">
        ...
        >
          <div class="basics">
            <div class="content">
              ...
              <el-form-item label="备注" prop="remarks" style="width: 66%;margin-left: 31px">
                  <el-input
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="focusMainDialogVisiblex = false">取 消</el-button>
...
export default {
  ...
  async mounted() {
    await positionList({}).then((res) => {
      this.tableDatax = res.data.data.pageData;
    });
    await queryeQuipmentList({}).then((res) => {
      let arr = res.data.data.pageData;
      arr.filter((item) => {
        if (item.id == this.$route.query.id) {
          this.form = item;
          this.key = true;
          this.equipmentTypexlist = this.form.equipmentType.split(",");
          this.tableDatax.forEach((v) => {
            this.equipmentTypexlist.forEach((c) => {
              if (c == v.id) {
          ...
          this.getsubid = this.form.subStationId;
          this.getsubStationIdx = this.form.subStationId;
          this.changeRediod = this.form;
          this.changeRediods = this.form;
          this.form.groupTypeList = this.filterOption(
            this.form.groupTypeId,
  ...
  methods: {
    filterOptionx(value, optionList) {
      let str = optionList.find((res) => {
        return res.id == value;
      });
      if (str) {
    ...
    deletes() {
      this.$confirm("操作无法撤回，确定要删除吗?").then(() => {
        removeEquipmentTemplate({ idList: [this.$route.query.id] }).then(() => {
          this.$router.push({ path: "./equipmentGroup" });
        });
      });
    },
    checkboxInit() {
      if (this.dialogformx.mainStationId.length > 0) {
        return true;
      } else {
        return false;
      }
    ...
    focusMainStation(num) {
      ...
      if (this.principaNum == 0) {
        ...
        console.log(this.dialogform.mainStationId, "sss");
    ...
    principalAdd() {
      ...
      if (this.changeRediod.hasOwnProperty("mainStationId")) {
        ...
        if (this.getsubStationId != "") {
          this.form.mainStationName = this.filterOptionx(
