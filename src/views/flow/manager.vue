    ...
    >
      ...
      <template slot-scope="scope" slot="menu">
        <el-button
          ...
          plain
          class="none-border"
          @click.stop="handlePermissionAssginment(scope.row,scope.index)"
        >权限配置</el-button>
        <el-button
          type="text"
          size="small"
      ...
      <template slot-scope="{row}" slot="suspensionState">
        <el-tag
          :type="row.suspensionState===1 ? 'success' : ''"
        >{{row.suspensionState===1?'激活':'挂起'}}</el-tag>
      </template>
...
export default {
  ...
  methods: {
    ...
    handlePermissionAssginment(row) {
      if (!row.formId || row.formId === 'null') {
        this.$message.warning("该流程未关联表单。");
        return;
      }
      this.$router.push(`/flow/manager/permission/${row.id}::${row.formId}`)
    },
    handleImage(row) {
