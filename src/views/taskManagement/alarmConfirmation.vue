    ...
    <el-card class="box-card">
      ...
      <div class="content">
        ...
        >
          ...
          <el-table-column label="工单编号" prop="wordOrderNo" align="center" />
          <el-table-column
            label="所属组织"
          ...
          <el-table-column label="设备品牌" prop="manufactor" align="center">
            <!-- <template slot-scope="scope">
          ...
          <el-table-column label="告警类型" prop="warningType" align="center">
            <!-- <template slot-scope="scope">
              {{ scope.row.taskClass | filterOptions(frequencyOptions) }}
            </template> -->
          </el-table-column>
          ...
          <el-table-column label="发起人" prop="initiatorName" align="center">
            <!-- <template slot-scope="scope">
              {{ scope.row.taskClass | filterOptions(frequencyOptions) }}
            </template> -->
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getQuerySumByOrderStatus() {
      alarmStateStatistics({}).then((res) => {
        this.stateInfo = res.data.data;
        console.log(this.stateInfo, res.data.data, ")))))O");
      });
    },
    // 工单发起时间下拉 - change
    changeOrderTime(val) {
    ...
    getQueryList(data) {
      queryAlarmConfirmdata(data).then((res) => {
    ...
    searchData() {
      this.getQueryList({
        ...
        wordOrderstates: this.form.wordOrderstate, //工单状态(字典)
        // wordOrderstate: "", //工单状态(字典)
        ...
        alarmLevels: this.form.alarmLevel, //最高告警级别(字典)
        // alarmLevel: "", //最高告警级别(字典)
    ...
    reset() {
      ...
      this.form.alarmLevel = []; //最高告警级别(字典)
      this.alarmTime = [];
      this.orderTime = [];
      this.sort1 = "no";
      this.sort2 = "no";
    ...
    deletes() {
      this.$confirm("操作无法撤回，确定要删除吗?").then(() => {
        remove({ id: this.multipleSelection }).then(() => {
          this.searchData();
        });
      });
    },
    ...
    handleExport() {
      exportList({
        ...
        equipmentCode: this.form.equipmentCode, //设别编码
        equipmentModel: this.form.equipmentModel, //规格型号
        alarmLevel: this.form.alarmLevel, //最高告警级别(字典)
        // alarmLevel: "", //最高告警级别(字典)
        orderColumn: this.orderColumn, //排序列 sort,region
        orderType: this.orderType, //排序类型 desc降序,asc升序
        pageNum: this.pageNum,
