        ...
        >
          <div class="basics">
            <div class="content">
              ...
              <div class="basice-box">
                ...
                <el-form-item label="终结时间:" prop="endTime">
                  <el-input
                    disabled
          ...
          <div class="sublevel">
            <div class="content">
              ...
              >
                ...
                >
                  <template scope="scope">
                    ...
                    >
                      <el-select
                        v-model="scope.row.taskContentName"
                        filterable
                        allow-create
                        :disabled="$route.query.id"
                        collapse-tags
                ...
                >
                  <template scope="scope">
                    <el-form-item
                      label-width="0"
                      :prop="'records.' + scope.$index + '.taskImgs'"
                    >
                      <div v-if="!scope.row.taskImgs||scope.row.taskImgs.length==0 ">
                        <el-tag>暂无图片上传!</el-tag>
                      </div>
                      <div>
                        <el-tag
                          v-for="item in scope.row.taskImgs"
                          :key="item"
                          closable
              ...
              <div class="add-box" @click="addRow" v-if="false">
...
import { queryProcessDetail } from "@/api/taskManagement/alarmConfirmation";
export default {
  name: "",
  props: {
    ...
    if (this.$route.query.id) {
      getDetail({ id: this.$route.query.id }).then((res) => {
        ...
        res.data.data.records.forEach((v) => {
          v.contentCescribe = v.contentDescribe;
          //  v.taskContentList
        });
        this.form.recordersName = Object.values(res.data.data.recorders);
      });
    } else {
      runProcess({ processInstanceId: this.processInstanceId }).then((res) => {
        this.form = res.data.data;
        res.data.data.records.forEach((v) => {
          v.contentCescribe = v.contentDescribe;
        });
  ...
  methods: {
    ...
    getTaskContent(val, index) {
      ...
      let code = "";
      this.form.records[index].taskContentName = "";
    ...
    handleClose(tag, data) {
      console.log( data.taskImgs,'?????????????');
      data.taskImgs.splice(data.taskImgs.indexOf(tag), 1);
    },
    handlePictureCardPreview(file) {
      ...
      if (
        ...
        file.url = this.imgUrl + file.fileId;
      } else {
    ...
    handleChange(file, fileList) {
      console.log(file, fileList, this.nowMsg,'xxxxxxxxxx');
      let regular = /^jpg|bmp|jpeg|png|gif|JPG|BMP|JPEG|PNG|GIF$/;
      if (regular.test(file.name.split(".")[file.name.split(".").length - 1])) {
        lrz(file.raw).then((file) => {
          imgUpload(file.origin)
            .then((res) => {
              ...
              if (!this.nowMsg.taskImgs) this.nowMsg.taskImgs = [];
              res.data.data.url = this.imgUrl + res.data.data.fileId;
              res.data.data.domain = this.imgUrl + res.data.data.fileId;
              this.nowMsg.taskImgs.push(res.data.data);
              this.$message.success("上传成功");
              this.form.records[indexs].taskImgs =
                this.nowMsg.taskImgs;
            })
            .catch(function (err) {
              // 处理失败会执行
              console.log(err);
        ...
        fileUpload(file.raw).then((res) => {
          ...
          res.data.data.fileName = res.data.data.name;
          this.nowMsg.taskImgs.push(res.data.data);
          this.form.records[indexs].taskImgs = this.nowMsg.taskImgs;
        });
      }
    },
    // 获取当前列表信息
