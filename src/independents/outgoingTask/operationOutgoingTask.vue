  ...
  <div>
    <el-card class="box-card">
      ...
      >
        <div class="form-box">
          ...
          <!-- <el-form-item label="任务区域" prop="destination">
            <el-input
              :disabled="($route.query.id && !listState) || !!processInstanceId"
          ...
          <el-form-item label="预计回站时间" prop="estimateReturnTime">
            <el-date-picker
              :disabled="
                ($route.query.id && !listState) ||
                  (!!processInstanceId && currentNode !== 0)
              "
              v-model="form.estimateReturnTime"
              type="datetime"
          ...
          <el-form-item label="车牌" prop="carNo">
            <el-input v-model="form.carNo" readonly disabled></el-input>
          </el-form-item>
          <el-form-item label="到达目的地时间" prop="arrivalTime">
            <el-input v-model="form.arrivalTime" readonly disabled></el-input>
          </el-form-item>
          <el-form-item label="路途耗时（h）" prop="wayTime">
            <el-input v-model="form.wayTime" readonly disabled></el-input>
          </el-form-item>
    ...
    >
      ...
      >
        ...
        <el-form-item label="任务区域" prop="destination" style="width: 50%">
          <el-input v-model="restartForm.destination" readonly></el-input>
        </el-form-item>
...
import {
  ...
  flowAbleComplete,
  updateStatus
} from "@/api/taskWorkOrder/outgoingTask.js";
...
// 任务状态
const taskState = [
  { value: "task_state_start", label: "开始" },
  { value: "task_state_pause", label: "暂停" },
  { value: "task_state_restart", label: "重启" },
  { value: "task_state_transfer_of_order", label: "转单" },
  { value: "task_state_end", label: "结束" },
  { value: "task_state_abandon", label: "废弃" }
];
// 流程节点
const processNode = {
  // 草稿
...
export default {
  ...
  data() {
    return {
      rules: {
        ...
        estimateReturnTime: [
          { required: true, message: "请选择预计回站时间", trigger: "change" }
        ],
        // taskDescribe: [
      ...
      form: {
        ...
        taskArea: "", // 任务区域id
        destination: "", // 目的地
        estimateReturnTime: "", //预计回站时间
        travelMode: "official_car", // 出行方式
        carNo: "", //车牌
        arrivalTime: "", //到达目的地时间
        wayTime: "", //路途耗时（h）
        workOrderNo: "", //工单编号
      ...
      travelModeOptions: JSON.parse(localStorage.getItem("travelModeOptions")), // 出行方式下拉
  ...
  methods: {
    ...
    record() {
      ...
      records({ id: this.form.id }).then(res => {
        this.$set(this.recordForm, "tableData", res.data.data);
      });
    },
    taskStateFormat(row) {
      let result = row.operationType;
      taskState.forEach(ele => {
        if (ele.value === row.operationType) {
          result = ele.label;
        }
      });
      return result;
    ...
    pause() {
      ...
      let param = { id, wordOrderState: "task_state_pause", taskLeader };
    ...
    deletes() {
      this.$confirm("操作无法撤回，确定要删除吗?").then(() => {
        remove({ idList: [this.form.id] }).then(() => {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          let param = {
            id,
            wordOrderState: "task_state_restart",
            taskLeader,
            remark
          };
          updateStatus(param).then(() => {
    ...
    areaAdd() {
      ...
      this.form.taskArea = this.$refs["areaRef"].chooseSelection
        .map(v => v.id)
        .join(",");
      this.form.destination = this.$refs["areaRef"].chooseSelection
        .map(v => v.areaName)
        .join(",");
    },
    // 上传任务图片
    ...
    beforeBtnClick(code) {
      ...
      if (code === "copySendBtnCode") {
        ...
        let param = {
          id: this.form.id,
          wordOrderState: "task_state_transfer_of_order",
      ...
      if (code === "disagreeBtnCode") {
        ...
        let param = {
          id: this.form.id,
          wordOrderState: "task_state_abandon"
        };
        updateStatus(param).then(() => {
