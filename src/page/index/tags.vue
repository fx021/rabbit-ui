...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      leaveFlag: state => state.common.checkLeave
    }),
    tagLen() {
  ...
  methods: {
    generateTitle(item) {
      // console.log(item)
     if(item.query.fromView=='DETAIL'){
       item.label='流程详情'
     }
     if(item.query.fromView == 'DEAL'){
       item.label='处理流程'
     }
     if(item.query.fromView=='DRAFT'){
       item.label='起草节点'
     }
    ...
    //激活当前选项
    setActive() {
      this.active = this.tag.value.split("?")[0];
    },
    ...
    checkLeave(callback) {
      if (this.leaveFlag) {
        this.$confirm('数据不会保存，确认要离开吗？', '离开确认')
          .then(() => {
    ...
    findTag(value) {
      ...
      return { tag: this.tagList[tagIndex], key: tagIndex };
    },
    closeAllTags() {
      this.checkLeave(() => {
        this.contextmenuFlag = false;
