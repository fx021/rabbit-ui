  ...
  <div class="widget-config">
    ...
    >
      <el-collapse v-model="collapse">
        <el-collapse-item name="3" title="业务字段">
           <el-form-item label="业务表">
        <el-select v-model="data.affair" placeholder="标签对齐方式">
          <el-option
            v-for="(item, index) in serviceTable"
            :key="index"
        ...
        <el-collapse-item name="1" title="基本属性">
          ...
          <el-form-item
            label="数据类型"
            v-if="
              ['cascader', 'checkbox', 'upload', 'img', 'array'].includes(
                data.type
              )
            "
          >
...
import { getNotification } from "../src/api/system/dict.js";
export default {
  name: "widget-config",
  props: ["data"],
  computed: {
  ...
  methods: {
    ...
    getNotification() {
      getNotification().then((res) => {
        this.serviceTable = res.data.data;
      });
    },
